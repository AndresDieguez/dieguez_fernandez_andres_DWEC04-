...
async function cargarDetalleCarta() {
  if (!cardId) {
      console.error('error al cargar la carta o no se proporcionó un ID de carta en la URL.');
  ...
  try {
      ...
      // obtenemos los detalles que necesitamos de la carta instanciando la clase Card y con ayuda de los metodos de CardAPI() 
...
function renderCardDetails(carta,data) {
  ...
  if (data.legalities && data.layout != "token" && data.layout != "art_series" && data.layout != "emblem" && data.layout != "emblem") {
    cardText += `<h3 class="mb-3 mt-3">Legal en los siguientes formatos:</h3>`;
  } else {
    cardText += `<h3 class="mb-3 mt-3">Esta carta No se juega</h3>`;
  }

  for (let formato in data.legalities) {
    if (data.legalities[formato] != `not_legal`) {
      ...
      <div class="col-12 col-md-6 col-lg-4 card-container">
        <div class="card-flip" id="flip-card">
          <div class="card-front">
            <img src="${frontImage}" alt="${data.name}" class="card-img-top">
          </div>
          <div class="card-back">
